...
static boost::format writeback_fmt("%-10s wb    %016x %02d %04d %02d  ");
...
void SimpleTracer::writeback(uint64_t cache_id, uint64_t addr, int32_t ai, int32_t s, int32_t w, const CMMetadataBase *meta, const CMDataBase *data) {
  std::cout << (writeback_fmt % UniqueID::name(cache_id) % addr % ai % s % w) ;
